...
pub fn main(init: std.process.Init) !void {
    ...
    const metadata_ok = vx.axial_cuda.enabled() and kernel != 0 and launch != 0 and registry.ok() and args.ok() and wrapped_launch.ok() and call_launch.ok() and top_level_call.ok() and scheduled_op.ok() and scheduled_op.scheduled() and partition.valid() and partition_metadata.valid() and partition_grid.x == 4 and typed_slice.valid();
    ...
    try stdout.interface.print(
        "{{\"kind\":\"vectra_axial_accelerator_smoke\",\"ok\":{},\"metadata_ok\":{},\"cuda_attempted\":{},\"cuda_launched\":{},\"kernel\":{d},\"launch\":{d},\"registry\":{d},\"wrapped_launch\":{d},\"call_launch\":{d},\"top_level_call\":{d},\"lazy_op\":{d},\"lazy_stream\":{d},\"partition\":{d},\"partition_grid_x\":{d},\"typed_slice\":{d},\"args\":{d},\"route\":\"{s}\",\"status\":\"{s}\",\"route_fingerprint\":{d}}}\n",
        .{ ok, metadata_ok, cuda_attempted, cuda_launched, kernel, launch, registry.fingerprint(), wrapped_launch.fingerprint(), call_launch.fingerprint(), top_level_call.fingerprint(), scheduled_op.fingerprint(), scheduled_op.stream.stream_id, partition_metadata.fingerprint(), partition_grid.x, typed_slice.fingerprint(), args.fingerprint(), route.route.label(), route.status.label(), route.fingerprint() },
    );
