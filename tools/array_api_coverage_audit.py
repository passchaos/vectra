...
KNOWN_GAPS: tuple[dict[str, Any], ...] = (
    {
        ...
        "reason": "Axiom now owns a real Metal/MPS device, command-queue, shared-buffer storage ABI, executable f32 2D Metal kernels for same-shape elementwise/scalar, unary math (abs/square/sqrt/exp/log/exp2/expm1/log1p/log2/log10/sin/cos/tan), matmul/matmulAdd, dot/matvec/vecmat, transpose, row/column broadcast add/sub/mul/div, reductions, softmax, and logSoftmax, plus f16 same-shape elementwise/scalar/abs/square/sqrt/exp, 2D matmul/matmulAdd, dot/matvec/vecmat, transpose, row/column broadcast add/sub/mul/div, sum/prod/min/max reductions, softmax, and logSoftmax, plus BF16 same-shape elementwise/scalar/abs/square/sqrt/exp, 2D matmul/matmulAdd, dot/matvec/vecmat, transpose, row/column broadcast add/sub/mul/div, sum/prod/min/max reductions, softmax, and logSoftmax covered by axiom-mps-storage-smoke; composed MPS coverage now also keeps f32/f16/BF16 rsqrt/leakyRelu/silu/hardsigmoid/hardswish/softshrink/elu/celu, powScalar(-1/-0.5/0/0.5/1/2/3), norm/normalize/cosineSimilarity/pairwiseDistance, and mse/l1/smoothL1/huber(.none) on real MPS storage. Remaining MPS dtypes/shapes still need runtime parity.",
        "target_layer": "axiom",
    },
    {
