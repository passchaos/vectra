...
KNOWN_GAPS: tuple[dict[str, Any], ...] = (
    {
        ...
        "reason": "Axiom now owns a real Metal/MPS device, command-queue, shared-buffer storage ABI, executable f32 2D Metal kernels for same-shape elementwise/scalar, unary math (abs/square/sqrt/exp/log/exp2/expm1/log1p/log2/log10/sin/cos/tan), matmul/matmulAdd, dot/matvec/vecmat, transpose, row/column broadcast add/sub/mul/div, reductions, softmax, logSoftmax, softmin, and logSoftmin, plus f16 same-shape elementwise/scalar/abs/square/sqrt/exp, 2D matmul/matmulAdd, dot/matvec/vecmat, transpose, row/column broadcast add/sub/mul/div, sum/prod/min/max reductions, softmax, logSoftmax, softmin, and logSoftmin, plus BF16 same-shape elementwise/scalar/abs/square/sqrt/exp, 2D matmul/matmulAdd, dot/matvec/vecmat, transpose, row/column broadcast add/sub/mul/div, sum/prod/min/max reductions, softmax, logSoftmax, softmin, and logSoftmin covered by axiom-mps-storage-smoke; composed MPS coverage now also keeps f32/f16/BF16 rsqrt/leakyRelu/silu/hardsigmoid/hardswish/softshrink/tanh/tanhshrink/elu/celu/selu/SELU/relu6/hardtanh/clipArray, powScalar(-1/-0.5/0/0.5/1/2/3), softmin/logSoftmin, norm/normalize/cosineSimilarity/pairwiseDistance, mean/variance/stddev, and mse/l1/smoothL1/huber(.none) on real MPS storage. Remaining MPS dtypes/shapes still need runtime parity.",
        "target_layer": "axiom",
    },
    {
