...
pub fn main(init: std.process.Init) !void {
    ...
    if (available) {
        ...
        const expected = [_]f32{ 1, 2, 3, 4, 16, 17, 22, 23, 9, 19, 11, 23, 14, 13, 16, 15 };

        var lhs = try vx.Array(f32).fromSliceOn(allocator, &lhs_values, &.{ 2, 2, 2, 2 }, vx.mps(0));
        defer lhs.deinit();
        var rhs = try vx.Array(f32).fromSliceOn(allocator, &rhs_values, &.{ 2, 2, 2, 2 }, vx.mps(0));
        defer rhs.deinit();
        ...
        f32_ok = out.device.isMps() and out.device_storage != null and
            std.mem.eql(usize, back.shape, &.{ 2, 2, 2, 2 }) and
            closeF32(back.data, &expected, 0.001);
        fingerprint ^= hashF32(back.data);

        var lhs16 = try vx.Array(f16).fromSliceOn(allocator, &toF16(lhs_values), &.{ 2, 2, 2, 2 }, vx.mps(0));
        defer lhs16.deinit();
        var rhs16 = try vx.Array(f16).fromSliceOn(allocator, &toF16(rhs_values), &.{ 2, 2, 2, 2 }, vx.mps(0));
        defer rhs16.deinit();
        ...
        f16_ok = out16.device.isMps() and out16.device_storage != null and
            std.mem.eql(usize, back16.shape, &.{ 2, 2, 2, 2 }) and
            closeF16(back16.data, &expected, 0.25);
        fingerprint ^= hashF16(back16.data);

        var lhs_bf16 = try vx.Array(vx.BFloat16).fromSliceOn(allocator, &toBF16(lhs_values), &.{ 2, 2, 2, 2 }, vx.mps(0));
        defer lhs_bf16.deinit();
        var rhs_bf16 = try vx.Array(vx.BFloat16).fromSliceOn(allocator, &toBF16(rhs_values), &.{ 2, 2, 2, 2 }, vx.mps(0));
        ...
        bf16_ok = out_bf16.device.isMps() and out_bf16.device_storage != null and
            std.mem.eql(usize, back_bf16.shape, &.{ 2, 2, 2, 2 }) and
            closeBF16(back_bf16.data, &expected, 0.5);
        fingerprint ^= hashBF16(back_bf16.data);
    }
    ...
    try stdout.interface.print(
        "{{\"kind\":\"vectra_axiom_mps_higher_rank_bmm_smoke\",\"ok\":{},\"available\":{},\"status\":\"{s}\",\"backend\":\"{s}\",\"f32_ok\":{},\"f16_ok\":{},\"bf16_ok\":{},\"fingerprint\":{d}}}\n",
...
fn hashF32(values: []const f32) u64 {
    var hasher = std.hash.Wyhash.init(0x4d50_5701_ba7c_4d32);
...
fn hashF16(values: []const f16) u64 {
    var hasher = std.hash.Wyhash.init(0x4d50_5701_ba7c_4d16);
    var len_bytes: [8]u8 = undefined;
    std.mem.writeInt(u64, &len_bytes, values.len, .little);
    hasher.update(&len_bytes);
    for (values) |value| {
...
fn hashBF16(values: []const vx.BFloat16) u64 {
    var hasher = std.hash.Wyhash.init(0x4d50_5701_ba7c_4db1);
    var len_bytes: [8]u8 = undefined;
    std.mem.writeInt(u64, &len_bytes, values.len, .little);
    hasher.update(&len_bytes);
    for (values) |value| {
