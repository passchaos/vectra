...
pub fn main(init: std.process.Init) !void {
    ...
    if (available) {
        ...
        var normal_a = try np.normalWith(f32, &.{9}, 0, 1, vx.seededOn(vx.mps(0), 1234));
        defer normal_a.deinit();
        var normal_b = try np.normalWith(f32, &.{9}, 0, 1, vx.seededOn(vx.mps(0), 1234));
        defer normal_b.deinit();
        var normal_constant = try np.normalWith(f32, &.{9}, 10, 0, vx.seededOn(vx.mps(0), 1234));
        defer normal_constant.deinit();
        var normal_a_back = try normal_a.cpu();
        defer normal_a_back.deinit();
        var normal_b_back = try normal_b.cpu();
        defer normal_b_back.deinit();
        var normal_constant_back = try normal_constant.cpu();
        defer normal_constant_back.deinit();
        var normal_f16_a = try np.normalWith(f16, &.{9}, 0, 1, vx.seededOn(vx.mps(0), 1234));
        defer normal_f16_a.deinit();
        var normal_f16_b = try np.normalWith(f16, &.{9}, 0, 1, vx.seededOn(vx.mps(0), 1234));
        defer normal_f16_b.deinit();
        var normal_f16_constant = try np.normalWith(f16, &.{9}, 10, 0, vx.seededOn(vx.mps(0), 1234));
        defer normal_f16_constant.deinit();
        var normal_f16_a_back = try normal_f16_a.cpu();
        defer normal_f16_a_back.deinit();
        var normal_f16_b_back = try normal_f16_b.cpu();
        defer normal_f16_b_back.deinit();
        var normal_f16_constant_back = try normal_f16_constant.cpu();
        defer normal_f16_constant_back.deinit();
        var normal_bf16_a = try np.normalWith(vx.BFloat16, &.{9}, vx.BFloat16.fromF32(0), vx.BFloat16.fromF32(1), vx.seededOn(vx.mps(0), 1234));
        defer normal_bf16_a.deinit();
        var normal_bf16_b = try np.normalWith(vx.BFloat16, &.{9}, vx.BFloat16.fromF32(0), vx.BFloat16.fromF32(1), vx.seededOn(vx.mps(0), 1234));
        defer normal_bf16_b.deinit();
        var normal_bf16_constant = try np.normalWith(vx.BFloat16, &.{9}, vx.BFloat16.fromF32(10), vx.BFloat16.fromF32(0), vx.seededOn(vx.mps(0), 1234));
        defer normal_bf16_constant.deinit();
        var normal_bf16_a_back = try normal_bf16_a.cpu();
        defer normal_bf16_a_back.deinit();
        var normal_bf16_b_back = try normal_bf16_b.cpu();
        defer normal_bf16_b_back.deinit();
        var normal_bf16_constant_back = try normal_bf16_constant.cpu();
        defer normal_bf16_constant_back.deinit();
        random_ok = random_a.device.isMps() and random_a.device_storage != null and
            ...
            normal_a.device.isMps() and normal_a.device_storage != null and
            normal_f16_a.device.isMps() and normal_f16_a.device_storage != null and
            normal_bf16_a.device.isMps() and normal_bf16_a.device_storage != null and
            ...
            inUnitRangeBF16(random_bf16_a_back.data) and
            equalF32(normal_a_back.data, normal_b_back.data) and
            !equalF32(normal_a_back.data, &.{ 0, 0, 0, 0, 0, 0, 0, 0, 0 }) and
            equalF32(normal_constant_back.data, &.{ 10, 10, 10, 10, 10, 10, 10, 10, 10 }) and
            equalF16(normal_f16_a_back.data, normal_f16_b_back.data) and
            !equalF16(normal_f16_a_back.data, &.{ 0, 0, 0, 0, 0, 0, 0, 0, 0 }) and
            equalF16(normal_f16_constant_back.data, &.{ 10, 10, 10, 10, 10, 10, 10, 10, 10 }) and
            equalBF16(normal_bf16_a_back.data, normal_bf16_b_back.data) and
            !equalBF16(normal_bf16_a_back.data, &.{
                vx.BFloat16.fromF32(0),
                vx.BFloat16.fromF32(0),
                vx.BFloat16.fromF32(0),
                vx.BFloat16.fromF32(0),
                vx.BFloat16.fromF32(0),
            ...
            closeBF16(normal_bf16_constant_back.data, &.{ 10, 10, 10, 10, 10, 10, 10, 10, 10 }, 0.125);
