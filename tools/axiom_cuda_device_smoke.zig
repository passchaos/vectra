...
pub fn main(init: std.process.Init) !void {
    ...
    var ok = !cuda_available or maybe_device != null;
    ...
    var direct_storage_ok = !cuda_available;
    var direct_shape_view_ok = !cuda_available;
    var direct_add_ok = !cuda_available;
    var direct_square_ok = !cuda_available;
    var direct_rand_f32_ok = !cuda_available;
    var direct_rand_f64_ok = !cuda_available;
    var direct_rand_f16_ok = !cuda_available;
    var direct_rand_bf16_ok = !cuda_available;
    var direct_unary_scalar_ok = !cuda_available;
    var direct_norm_ok = !cuda_available;
    var direct_reduction_ok = !cuda_available;
    var direct_broadcast_ok = !cuda_available;
    var direct_transpose_ok = !cuda_available;
    var direct_softmax_ok = !cuda_available;
    var direct_log_softmax_ok = !cuda_available;
    var direct_ternary_ok = !cuda_available;
    var direct_matmul_ok = !cuda_available;
    var direct_bmm_ok = !cuda_available;
    var direct_batched_matmul_ok = !cuda_available;
    var direct_higher_rank_batched_matmul_ok = !cuda_available;
    var direct_broadcasted_batched_matmul_ok = !cuda_available;
    var direct_mixed_batched_matmul_ok = !cuda_available;
    var direct_rank3_scalar_broadcast_ok = !cuda_available;
    var direct_rank3_lastdim_broadcast_ok = !cuda_available;
    var direct_rank3_middle_broadcast_ok = !cuda_available;
    var direct_rank3_reduction_ok = !cuda_available;
    var direct_batched_matvec_ok = !cuda_available;
    var direct_batched_vecmat_ok = !cuda_available;
    var direct_dot_ok = !cuda_available;
    var direct_inner_ok = !cuda_available;
    var direct_outer_ok = !cuda_available;
    var direct_matmul_add_ok = !cuda_available;
    var scaled_matmul_add_ok = !cuda_available;
    var chained_matmul_add_ok = !cuda_available;
    var chained_matmul_sub_ok = !cuda_available;
    var chained_sqrt_ok = !cuda_available;
    var chained_exp_ok = !cuda_available;
    var reversed_add_fusion_ok = !cuda_available;
    var reversed_sub_fusion_ok = !cuda_available;
    var pending_fusion_status_ok = !cuda_available;
    var bf16_chained_sqrt_ok = !cuda_available;
    var bf16_chained_exp_ok = !cuda_available;
    var bf16_bmm_ok = !cuda_available;
    var bf16_scalar_mul_ok = !cuda_available;
    var bf16_broadcast_ok = !cuda_available;
    var bf16_reduction_ok = !cuda_available;
    var bf16_transpose_ok = !cuda_available;
    var bf16_softmax_ok = !cuda_available;
    var bf16_log_softmax_ok = !cuda_available;
    var f16_activation_ok = !cuda_available;
    var f16_bmm_ok = !cuda_available;
    var f16_broadcast_ok = !cuda_available;
    var f16_reduction_ok = !cuda_available;
    var f16_transpose_ok = !cuda_available;
    var f16_softmax_ok = !cuda_available;
    var f16_log_softmax_ok = !cuda_available;
    var f64_matmul_ok = !cuda_available;
    var f64_bmm_ok = !cuda_available;
    var f64_elementwise_ok = !cuda_available;
    var f64_transpose_ok = !cuda_available;
    var f64_broadcast_ok = !cuda_available;
    var f64_reduction_ok = !cuda_available;
    var f64_softmax_ok = !cuda_available;
    var f64_log_softmax_ok = !cuda_available;
    var f64_matmul_add_ok = !cuda_available;
    ...
    var batched_gemm_backend: []const u8 = "";
    var mixed_batched_gemm_backend: []const u8 = "";
    ...
    var scalar_api_memref_fingerprint: u64 = 0;
    ...
    if (cuda_available) {
        ...
        var cuda_rand_f32 = try np.randWith(f32, &.{8}, vx.seededOn(vx.cuda(0), 1234));
        defer cuda_rand_f32.deinit();
        var cuda_rand_f32_host = try cuda_rand_f32.cpu();
        defer cuda_rand_f32_host.deinit();
        var cuda_rand_f64 = try np.randWith(f64, &.{8}, vx.seededOn(vx.cuda(0), 1234));
        defer cuda_rand_f64.deinit();
        var cuda_rand_f64_host = try cuda_rand_f64.cpu();
        defer cuda_rand_f64_host.deinit();
        var cuda_rand_f16 = try np.randWith(f16, &.{8}, vx.seededOn(vx.cuda(0), 1234));
        defer cuda_rand_f16.deinit();
        var cuda_rand_f16_host = try cuda_rand_f16.cpu();
        defer cuda_rand_f16_host.deinit();
        var cuda_rand_bf16 = try np.randWith(vx.BFloat16, &.{8}, vx.seededOn(vx.cuda(0), 1234));
        defer cuda_rand_bf16.deinit();
        var cuda_rand_bf16_host = try cuda_rand_bf16.cpu();
        defer cuda_rand_bf16_host.deinit();
        direct_rand_f32_ok = cuda_rand_f32.device_storage != null and inRangeF32(cuda_rand_f32_host.data);
        direct_rand_f64_ok = cuda_rand_f64.device_storage != null and inRangeF64(cuda_rand_f64_host.data);
        direct_rand_f16_ok = cuda_rand_f16.device_storage != null and inRangeF16(cuda_rand_f16_host.data);
        direct_rand_bf16_ok = cuda_rand_bf16.device_storage != null and inRangeBF16(cuda_rand_bf16_host.data);
        ...
        var all_axes_mean = try lhs.meanAxes(&.{ 0, 1 }, false);
        defer all_axes_mean.deinit();
        var all_axes_mean_host = try all_axes_mean.cpu();
        defer all_axes_mean_host.deinit();
        var all_axes_mean_keep = try lhs.meanAxes(&.{ 0, 1 }, true);
        defer all_axes_mean_keep.deinit();
        var all_axes_mean_keep_host = try all_axes_mean_keep.cpu();
        defer all_axes_mean_keep_host.deinit();
        var flat_var = try lhs.variance(null, false, 0.0);
        defer flat_var.deinit();
        var flat_var_host = try flat_var.cpu();
        defer flat_var_host.deinit();
        var flat_std_keep = try lhs.stddev(null, true, 0.0);
        defer flat_std_keep.deinit();
        var flat_std_keep_host = try flat_std_keep.cpu();
        defer flat_std_keep_host.deinit();
        var all_axes_var = try lhs.varianceAxes(&.{ 0, 1 }, false, 0.0);
        ...
        direct_reduction_ok = row_sum.device.isCuda() and row_sum.device_storage != null and
            ...
            equalF32(flat_mean_keep_host.data, &.{2.5}) and
            all_axes_mean.device.isCuda() and all_axes_mean.device_storage != null and
            std.mem.eql(usize, all_axes_mean_host.shape, &.{}) and
            equalF32(all_axes_mean_host.data, &.{2.5}) and
            all_axes_mean_keep.device.isCuda() and all_axes_mean_keep.device_storage != null and
            std.mem.eql(usize, all_axes_mean_keep_host.shape, &.{ 1, 1 }) and
            equalF32(all_axes_mean_keep_host.data, &.{2.5}) and
            flat_var.device.isCuda() and flat_var.device_storage != null and
            std.mem.eql(usize, flat_var_host.shape, &.{}) and
            approxF32(flat_var_host.data[0], 1.25, 0.0001) and
            flat_std_keep.device.isCuda() and flat_std_keep.device_storage != null and
            std.mem.eql(usize, flat_std_keep_host.shape, &.{ 1, 1 }) and
        ...
        direct_rank3_scalar_broadcast_ok = rank3_scalar_div.device.isCuda() and
            ...
            std.mem.eql(u8, rank3_scalar_report.operation, "broadcast_binary2d") and
            std.mem.eql(usize, rank3_scalar_div_host.shape, &.{ 1, 2, 2 }) and
            approxF32(rank3_scalar_div_host.data[0], 0.5, 0.0001) and
            approxF32(rank3_scalar_div_host.data[1], 1.0, 0.0001) and
            approxF32(rank3_scalar_div_host.data[2], 1.5, 0.0001) and
            approxF32(rank3_scalar_div_host.data[3], 2.0, 0.0001);
        var lastdim_bias = try vx.Array(f32).fromSliceOn(allocator, &.{ 10, 20 }, &.{2}, vx.cuda(0));
        defer lastdim_bias.deinit();
        var rank3_lastdim_div = try rank3.div(lastdim_bias);
        defer rank3_lastdim_div.deinit();
        var rank3_lastdim_div_host = try rank3_lastdim_div.cpu();
        defer rank3_lastdim_div_host.deinit();
        var rank3_lastdim_rsub = try lastdim_bias.sub(rank3);
        ...
        direct_rank3_lastdim_broadcast_ok = rank3_lastdim_div.device.isCuda() and
            ...
            std.mem.eql(u8, rank3_lastdim_report.operation, "broadcast_binary2d") and
            std.mem.eql(usize, rank3_lastdim_div_host.shape, &.{ 1, 2, 2 }) and
            approxF32(rank3_lastdim_div_host.data[0], 0.1, 0.0001) and
            approxF32(rank3_lastdim_div_host.data[1], 0.1, 0.0001) and
            approxF32(rank3_lastdim_div_host.data[2], 0.3, 0.0001) and
        ...
        var middle_lhs = try vx.Array(f32).fromSliceOn(allocator, &.{ 1, 2, 3, 4 }, &.{ 2, 1, 2 }, vx.cuda(0));
        defer middle_lhs.deinit();
        var middle_rhs = try vx.Array(f32).fromSliceOn(allocator, &.{ 10, 20, 30, 40 }, &.{ 1, 2, 2 }, vx.cuda(0));
        defer middle_rhs.deinit();
        var middle_broadcast = try middle_lhs.add(middle_rhs);
        ...
        direct_rank3_middle_broadcast_ok = middle_broadcast.device.isCuda() and
            ...
            std.mem.eql(u8, middle_broadcast_report.operation, "broadcast4_f32") and
            std.mem.eql(usize, middle_broadcast_host.shape, &.{ 2, 2, 2 }) and
            equalF32(middle_broadcast_host.data, &.{ 11, 22, 31, 42, 13, 24, 33, 44 });
        ...
        direct_broadcast_ok = row_broadcast.device.isCuda() and row_broadcast.device_storage != null and
            ...
            scalar_broadcast_div.device.isCuda() and scalar_broadcast_div.device_storage != null and
            scalar_broadcast_report.valid() and
            std.mem.eql(u8, scalar_broadcast_report.operation, "broadcast_binary2d") and
            approxF32(scalar_broadcast_div_host.data[0], 0.5, 0.0001) and
        ...
        const scalar_api_report = vx.axiom_backend.cuda.lastCudaDeviceMemRefReport();
        scalar_api_memref_fingerprint = scalar_api_report.memref_spec_fingerprint;
        ...
        var row_norm = try lhs.norm(2.0, 1, false);
        ...
        direct_unary_scalar_ok = negated.device.isCuda() and negated.device_storage != null and
            ...
            shifted_for_relu.device.isCuda() and shifted_for_relu.device_storage != null and
            scalar_api_report.valid() and
            std.mem.eql(u8, scalar_api_report.operation, "broadcast_binary2d") and
        ...
        var batch_matmul_product = try batch_lhs.matmul(batch_rhs);
        ...
        var broadcast_batch_lhs = try vx.Array(f32).fromSliceOn(allocator, &.{
            1, 2,
            3, 4,
        }, &.{ 1, 2, 2 }, vx.cuda(0));
        defer broadcast_batch_lhs.deinit();
        var broadcast_batch_rhs = try vx.Array(f32).fromSliceOn(allocator, &.{
            1, 0,
            0, 1,
        ...
        defer broadcast_batch_rhs.deinit();
        var broadcast_batch_product = try broadcast_batch_lhs.matmul(broadcast_batch_rhs);
        ...
        direct_broadcasted_batched_matmul_ok = broadcast_batch_product.device.isCuda() and
            ...
            broadcasted_batched_report.batch_count == 2 and
            std.mem.eql(u8, broadcasted_batched_report.backend, batched_gemm_backend) and
            std.mem.eql(usize, broadcast_batch_host.shape, &.{ 2, 2, 2 }) and
            equalF32(broadcast_batch_host.data, &.{ 1, 2, 3, 4, 4, 5, 10, 11 });

        var mixed_batch_lhs = try vx.Array(f32).fromSliceOn(allocator, &.{
            1, 2,
            3, 4,
        ...
        defer mixed_batch_lhs.deinit();
        var mixed_batch_rhs = try vx.Array(f32).fromSliceOn(allocator, &.{
            1, 0,
            0, 1,
        ...
        defer mixed_batch_rhs.deinit();
        var mixed_batch_product = try mixed_batch_lhs.matmul(mixed_batch_rhs);
        ...
        direct_mixed_batched_matmul_ok = mixed_batch_product.device.isCuda() and
            ...
            mixed_batched_report.valid() and
            mixed_batched_report.batch_count == 6 and
            std.mem.eql(u8, mixed_batched_report.backend, "loop_over_gemm_memrefs") and
            std.mem.eql(usize, mixed_batch_host.shape, &.{ 2, 3, 2, 2 }) and
            equalF32(mixed_batch_host.data, &.{
                1,  2,  3,  4,
                4,  5,  10, 11,
                1,  3,  3,  7,
                5,  6,  7,  8,
        ...
        var batched_vec = try vx.Array(f32).fromSliceOn(allocator, &.{ 10, 20 }, &.{2}, vx.cuda(0));
        defer batched_vec.deinit();
        var batched_matvec = try batch_lhs.matmul(batched_vec);
        ...
        direct_batched_matvec_ok = batched_matvec.device.isCuda() and
            ...
            batched_matvec_report.batch_count == 2 and
            std.mem.eql(u8, batched_matvec_report.backend, batched_gemm_backend) and
            std.mem.eql(usize, batched_matvec_host.shape, &.{ 2, 2 }) and
            equalF32(batched_matvec_host.data, &.{ 50, 110, 170, 230 });

        var batched_vecmat = try batched_vec.matmul(batch_rhs);
        ...
        direct_batched_vecmat_ok = batched_vecmat.device.isCuda() and
            ...
            batched_vecmat_report.batch_count == 2 and
            std.mem.eql(u8, batched_vecmat_report.backend, batched_gemm_backend) and
            std.mem.eql(usize, batched_vecmat_host.shape, &.{ 2, 2 }) and
            equalF32(batched_vecmat_host.data, &.{ 10, 20, 40, 50 });

        var dot_lhs = try vx.Array(f32).fromSliceOn(allocator, &.{ 1, 2, 3 }, &.{3}, vx.cuda(0));
        defer dot_lhs.deinit();
        var dot_rhs = try vx.Array(f32).fromSliceOn(allocator, &.{ 10, 20, 30 }, &.{3}, vx.cuda(0));
        defer dot_rhs.deinit();
        var dot_out = try dot_lhs.dot(dot_rhs);
        ...
        direct_dot_ok = dot_out.device.isCuda() and
            ...
            dot_report.memref_spec_fingerprint != 0 and
            std.mem.eql(usize, dot_host.shape, &.{}) and
            equalF32(dot_host.data, &.{140}) and
            vdot_out.device.isCuda() and
            vdot_out.device_storage != null and
            vdot_report.valid() and
            std.mem.eql(usize, vdot_host.shape, &.{}) and
            equalF32(vdot_host.data, &.{140});
        ...
        direct_inner_ok = inner_out.device.isCuda() and
            ...
            inner_report.memref_spec_fingerprint != 0 and
            std.mem.eql(usize, inner_host.shape, &.{ 2, 2 }) and
            equalF32(inner_host.data, &.{ 140, 6, 320, 15 });

        var outer_lhs = try vx.Array(f32).fromSliceOn(allocator, &.{ 1, 2, 3 }, &.{3}, vx.cuda(0));
        defer outer_lhs.deinit();
        var outer_rhs = try vx.Array(f32).fromSliceOn(allocator, &.{ 10, 20 }, &.{2}, vx.cuda(0));
        ...
        direct_outer_ok = outer_out.device.isCuda() and
            ...
            outer_report.memref_spec_fingerprint != 0 and
            std.mem.eql(usize, outer_host.shape, &.{ 3, 2 }) and
            equalF32(outer_host.data, &.{ 10, 20, 20, 40, 30, 60 });
        ...
        direct_matmul_add_ok = fused.device.isCuda() and fused.device_storage != null and
            ...
            matmul_add_report.memref_spec_fingerprint != 0 and
            equalF32(fused_host.data, &.{ 4, 4, 8, 8 });
    ...
    const memref_fingerprints_ok = !cuda_available or
        (elementwise_binary_memref_fingerprint != 0 and
            ...
            scalar_api_memref_fingerprint != 0 and
    ...
    try stdout.interface.print(
        "{{\"kind\":\"vectra_axiom_cuda_device_smoke\",\"enabled\":{},\"status\":\"{s}\",\"ok\":{},\"bytes\":{d},\"fingerprint\":{d},\"direct_storage_ok\":{},\"direct_shape_view_ok\":{},\"direct_add_ok\":{},\"direct_square_ok\":{},\"direct_rand_f32_ok\":{},\"direct_rand_f64_ok\":{},\"direct_rand_f16_ok\":{},\"direct_rand_bf16_ok\":{},\"direct_unary_scalar_ok\":{},\"direct_norm_ok\":{},\"direct_reduction_ok\":{},\"direct_broadcast_ok\":{},\"direct_transpose_ok\":{},\"direct_softmax_ok\":{},\"direct_log_softmax_ok\":{},\"direct_ternary_ok\":{},\"direct_matmul_ok\":{},\"direct_matmul_add_ok\":{},\"scaled_matmul_add_ok\":{},\"chained_matmul_add_ok\":{},\"chained_matmul_sub_ok\":{},\"chained_sqrt_ok\":{},\"chained_exp_ok\":{},\"reversed_add_fusion_ok\":{},\"reversed_sub_fusion_ok\":{},\"pending_fusion_status_ok\":{}",
        .{ vx.axiom_backend.cuda.enabled(), status, ok, bytes, fingerprint, direct_storage_ok, direct_shape_view_ok, direct_add_ok, direct_square_ok, direct_rand_f32_ok, direct_rand_f64_ok, direct_rand_f16_ok, direct_rand_bf16_ok, direct_unary_scalar_ok, direct_norm_ok, direct_reduction_ok, direct_broadcast_ok, direct_transpose_ok, direct_softmax_ok, direct_log_softmax_ok, direct_ternary_ok, direct_matmul_ok, direct_matmul_add_ok, scaled_matmul_add_ok, chained_matmul_add_ok, chained_matmul_sub_ok, chained_sqrt_ok, chained_exp_ok, reversed_add_fusion_ok, reversed_sub_fusion_ok, pending_fusion_status_ok },
    );
    try stdout.interface.print(
        ",\"bf16_chained_sqrt_ok\":{},\"bf16_chained_exp_ok\":{},\"bf16_scalar_mul_ok\":{},\"bf16_broadcast_ok\":{},\"bf16_reduction_ok\":{},\"direct_rank3_scalar_broadcast_ok\":{},\"direct_rank3_lastdim_broadcast_ok\":{},\"bf16_transpose_ok\":{},\"bf16_softmax_ok\":{},\"bf16_bmm_ok\":{},\"bf16_log_softmax_ok\":{},\"f16_activation_ok\":{},\"f16_bmm_ok\":{},\"f16_broadcast_ok\":{},\"f16_reduction_ok\":{},\"f16_transpose_ok\":{},\"f16_softmax_ok\":{},\"f16_log_softmax_ok\":{},\"f64_matmul_ok\":{},\"f64_bmm_ok\":{},\"direct_bmm_ok\":{},\"direct_batched_matmul_ok\":{},\"direct_higher_rank_batched_matmul_ok\":{},\"direct_broadcasted_batched_matmul_ok\":{},\"direct_mixed_batched_matmul_ok\":{},\"direct_batched_matvec_ok\":{},\"direct_batched_vecmat_ok\":{},\"direct_dot_ok\":{},\"direct_inner_ok\":{},\"direct_outer_ok\":{},\"f64_elementwise_ok\":{},\"f64_transpose_ok\":{}",
        .{ bf16_chained_sqrt_ok, bf16_chained_exp_ok, bf16_scalar_mul_ok, bf16_broadcast_ok, bf16_reduction_ok, direct_rank3_scalar_broadcast_ok, direct_rank3_lastdim_broadcast_ok, bf16_transpose_ok, bf16_softmax_ok, bf16_bmm_ok, bf16_log_softmax_ok, f16_activation_ok, f16_bmm_ok, f16_broadcast_ok, f16_reduction_ok, f16_transpose_ok, f16_softmax_ok, f16_log_softmax_ok, f64_matmul_ok, f64_bmm_ok, direct_bmm_ok, direct_batched_matmul_ok, direct_higher_rank_batched_matmul_ok, direct_broadcasted_batched_matmul_ok, direct_mixed_batched_matmul_ok, direct_batched_matvec_ok, direct_batched_vecmat_ok, direct_dot_ok, direct_inner_ok, direct_outer_ok, f64_elementwise_ok, f64_transpose_ok },
    );
    try stdout.interface.print(
        ",\"f64_broadcast_ok\":{},\"f64_reduction_ok\":{},\"f64_softmax_ok\":{},\"f64_log_softmax_ok\":{},\"f64_matmul_add_ok\":{},\"direct_rank3_middle_broadcast_ok\":{},\"direct_rank3_reduction_ok\":{},\"memref_fingerprints_ok\":{},\"elementwise_binary_memref_fingerprint\":{d},\"elementwise_unary_memref_fingerprint\":{d},\"gemm_memref_fingerprint\":{d},\"batched_gemm_backend\":\"{s}\",\"mixed_batched_gemm_backend\":\"{s}\",\"batched_gemm_fingerprint\":{d},\"f64_gemm_memref_fingerprint\":{d},\"matmul_add_memref_fingerprint\":{d},\"matmul_add_unary_memref_fingerprint\":{d},\"scalar_api_memref_fingerprint\":{d},\"reduction_memref_fingerprint\":{d},\"broadcast_memref_fingerprint\":{d},\"transpose_memref_fingerprint\":{d},\"softmax_memref_fingerprint\":{d},\"log_softmax_memref_fingerprint\":{d}}}\n",
        .{ f64_broadcast_ok, f64_reduction_ok, f64_softmax_ok, f64_log_softmax_ok, f64_matmul_add_ok, direct_rank3_middle_broadcast_ok, direct_rank3_reduction_ok, memref_fingerprints_ok, elementwise_binary_memref_fingerprint, elementwise_unary_memref_fingerprint, gemm_memref_fingerprint, batched_gemm_backend, mixed_batched_gemm_backend, batched_gemm_fingerprint, f64_gemm_memref_fingerprint, matmul_add_memref_fingerprint, matmul_add_unary_memref_fingerprint, scalar_api_memref_fingerprint, reduction_memref_fingerprint, broadcast_memref_fingerprint, transpose_memref_fingerprint, softmax_memref_fingerprint, log_softmax_memref_fingerprint },
    );
...
fn inRangeF32(values: []const f32) bool {
    for (values) |value| {
...
fn inRangeF64(values: []const f64) bool {
    for (values) |value| {
        if (!(value >= 0 and value < 1)) return false;
    }
    return true;
...
fn inRangeF16(values: []const f16) bool {
    for (values) |value| {
        const casted: f32 = @floatCast(value);
        if (!(casted >= 0 and casted < 1)) return false;
    }
    return true;
}

fn inRangeBF16(values: []const vx.BFloat16) bool {
    for (values) |value| {
        const casted = value.toF32();
        if (!(casted >= 0 and casted < 1)) return false;
    }
    return true;
}
