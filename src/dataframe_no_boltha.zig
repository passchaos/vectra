...
pub const DeviceDataFrameView = struct {
    ...
    pub fn columnDTypes(self: DeviceDataFrameView, allocator: std.mem.Allocator) std.mem.Allocator.Error![]DeviceDType {
        const out = try allocator.alloc(DeviceDType, self.columns.len);
        for (self.columns, out) |column_value, *slot| slot.* = column_value.dtype;
        return out;
    }

    pub fn dtypes(self: DeviceDataFrameView, allocator: std.mem.Allocator) std.mem.Allocator.Error![]DeviceDType {
