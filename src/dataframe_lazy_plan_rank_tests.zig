...
test "device lazy frame collects plan operations" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, explained, "raw_ops=93") != null);
    try std.testing.expect(std.mem.indexOf(u8, explained, "optimized_ops=91") != null);
    ...
    try std.testing.expectEqual(@as(usize, 89), result.width());
