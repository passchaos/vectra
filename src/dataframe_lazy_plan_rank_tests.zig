...
test "device lazy frame derives row numeric reduction columns" {
    ...
    var result = try plan.collect();
    defer result.deinit();
    try std.testing.expectEqual(@as(usize, 108), result.width());
