...
test "device lazy frame collects plan operations" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, explained, "raw_ops=90") != null);
    try std.testing.expect(std.mem.indexOf(u8, explained, "optimized_ops=88") != null);
    ...
    try std.testing.expectEqual(@as(usize, 86), result.width());
