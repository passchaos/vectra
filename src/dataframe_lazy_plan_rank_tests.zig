...
test "device lazy frame collects plan operations" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, explained, "raw_ops=91") != null);
    try std.testing.expect(std.mem.indexOf(u8, explained, "optimized_ops=89") != null);
    ...
    try std.testing.expectEqual(@as(usize, 87), result.width());
