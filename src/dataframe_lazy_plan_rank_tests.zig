...
test "device lazy frame collects plan operations" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, explained, "raw_ops=80") != null);
    try std.testing.expect(std.mem.indexOf(u8, explained, "optimized_ops=78") != null);
    ...
    try std.testing.expectEqual(@as(usize, 76), result.width());
