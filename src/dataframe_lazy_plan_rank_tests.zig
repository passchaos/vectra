...
test "device lazy frame collects plan operations" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, explained, "raw_ops=69") != null);
    try std.testing.expect(std.mem.indexOf(u8, explained, "optimized_ops=67") != null);
    ...
    try std.testing.expectEqual(@as(usize, 65), result.width());
