...
test "device lazy frame collects plan operations" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, explained, "raw_ops=85") != null);
    try std.testing.expect(std.mem.indexOf(u8, explained, "optimized_ops=83") != null);
    ...
    try std.testing.expectEqual(@as(usize, 81), result.width());
