...
test "device lazy frame collects plan operations" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, explained, "raw_ops=65") != null);
    try std.testing.expect(std.mem.indexOf(u8, explained, "optimized_ops=63") != null);
    ...
    try std.testing.expectEqual(@as(usize, 61), result.width());
