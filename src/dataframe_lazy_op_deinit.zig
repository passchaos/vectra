...
pub fn deinit(comptime Self: type, self: *Self, allocator: std.mem.Allocator) void {
    switch (self.*) {
        ...
        .drop_nulls, .drop_nans, .drop_infs, .drop_non_finites => |names| freeNameList(allocator, names),
        .filter_nulls_column, .filter_nans_column, .filter_infs_column, .filter_non_finites_column => |name| allocator.free(name),
