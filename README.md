...
`totalNbytes`, `dataMemoryUsage`, `validityMemoryUsage`, `memoryUsage`, `estimatedSize`, `columnSchema`/`columnSchemaAt`/`columnSchemas`/`schema`/`schemaSummary` with per-column dtype/null-validity/memory/device/shape/schema-compatibility helpers, `equals`/`frameEquals`, `allClose`/`frameAllClose`, `schemaEquals`/`sameSchema`/`schemaCompatible`, `deviceBackendName`, `isCpu`,
...
also expose `vx.ArrowExport.Column.toArrowField`, `ColumnSchema.toArrowField`,
`vx.ArrowExport.ColumnView.toArrowField`, and
`vx.ArrowExport.DataFrameView.toArrowFields`/`toArrowSchema` for exporting
schema and non-owning view metadata directly (with the older `Device*Arrow`
