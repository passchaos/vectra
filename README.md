...
- `ArrayView(T)` also exposes materializing indexing/search/shape wrappers such as `repeatInterleave/repeatInterleaveScalar`, `take/takeSigned`, `indexSelect/indexSelectSigned`, `gather/gatherSigned`, `maskedSelect`, `compress`, `nonzero/argwhere/whereIndices`, `unique`, `searchsorted`, `bucketize`, `digitize`, and `isin`.
