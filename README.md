...
select/positional-select-drop/first-last-column-select/name-pattern-select/name-pattern-drop/dtype-select/dtype-drop/cast/fill-null/coalesce-column/drop-null/filter-null/null-predicate-column/literal-column/positioned-literal-column/copy-column/row-index/rename/bulk-rename/move/drop/boolean-column-filter/scalar-filter/filter-mask/sort/reverse/slice/stepped-slice/stride/sample/replacement-sample/take/head/tail operations over either an eager
dataframe source or a `DeviceParquetScan` source, exposes `explain()`, folds
adjacent select/head/tail operations and exposes lazy row reversal/slicing/stepped-slicing/striding/sampling/replacement-sampling/gathering, rewrites adjacent sort+head into `topKBy`,
...
`dropNumeric`, `dropFloat`, `dropInteger`, `dropBool`, `castColumn`,
`fillNullColumn`, `coalesceColumns`, `dropNulls`, `filterNullsColumn`, `isNullColumn`, `isValidColumn`, `withColumnAt`, `withColumnBefore`, `withColumnAfter`,
...
`moveColumn`, `moveColumnBefore`, `moveColumnAfter`, `dropColumn`, `compareColumnScalar`, `addColumns`, `filterColumn`, and `filterColumnMask` reuse Vectra `Array` operations, so supported dtypes route
