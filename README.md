...
- `Series(T)` and heterogeneous CPU `DataFrame` with select/filter/sort/head/tail/describe/group-by-sum, plus a cuDF/Polars-inspired `DeviceDataFrame`/`DeviceColumn` owning fixed-width columns on a shared Vectra device (CPU/CUDA/MPS) with non-owning table/column views, row concat/append/vstack, row distinct/drop-duplicates, nullable validity metadata, eager same-dtype column arithmetic/comparisons, generated boolean-mask filtering, lightweight `DeviceLazyFrame` dataframe or Parquet-scan sources with derived-column expressions, lazy groupby aggregations, lazy equi/asof joins, lazy row concat/append/vstack, lazy distinct/drop-duplicates, select/scalar-filter/filter-mask/sort/head/tail plans, explain/scan-pushdown/adjacent-op/top-k optimization, stable argsort/sort-by-key/top-k with null placement, `rankProfileBy` rank/window columns (ordinal/competition/dense/percent/cume-dist), `bucketProfile` empirical distribution columns (ECDF/equal-frequency bucket/lower-tail/upper-tail), `validityProfile` data-quality columns (is-null/is-valid/valid-streak/null-streak), `classificationProfile` bool evaluation columns (TP/FP/TN/FN/correct), `boolTransitionProfile` bool sequence columns (rising/falling/toggled/true-streak/false-streak), `rollingBoolProfile` trailing bool-window columns (true-count/false-count/true-rate/any/all), `clipProfile` bounded-cleaning columns (clipped/below/above/in-range), `thresholdProfile` threshold-distance columns (distance/absolute/above/below/at), `emaProfile` exponential-smoothing columns (EMA/residual/ratio), `linearFitProfile` two-column model diagnostics (fitted/residual/residual-z/slope), `rollingLinearFitProfile` trailing two-column regression diagnostics (pair-count/slope/intercept/fitted/residual/residual-z), `errorProfile` forecast/error diagnostics (error/absolute/squared/APE/SMAPE), `rollingCorrelationProfile` two-column rolling statistics (pair-count/covariance/correlation/beta), `rollingProfile` trailing-window columns (count/sum/mean/variance/stddev), `rollingMomentProfile` trailing higher-moment columns (count/M3/M4/skewness/kurtosis), `rollingRangeProfile` trailing range columns (low/high/range/position), `rollingNormalizeProfile` trailing normalization columns (centered/z-score/min-max), `rollingQuantileProfile` trailing distribution columns (q1/median/q3/IQR), `rollingDrawdownProfile` trailing risk columns (peak/drawdown/drawdown-pct/peak-age), `lagProfile`/`leadProfile` sequence feature columns (lag/lead, backward/forward diff, pct-change), `signProfile` sign-state columns (sign/sign-flip/positive/negative/zero streak), `expandingProfile` cumulative columns (count/sum/mean/min/max), `expandingMomentProfile` cumulative higher-moment columns (count/M3/M4/skewness/kurtosis), `extremaProfile` running extrema columns (low/high/new-low/new-high), `standardizeProfile` scale columns (centered/z-score/min-max), `robustProfile` outlier-resistant columns (median-centered/MAD-z/IQR-outlier/winsorized), `drawdownProfile` risk/time-series columns (running-peak/drawdown/drawdown-pct), `trendProfile` sequence columns (trend/up-streak/down-streak/flat-streak/reversal), `crossoverProfile` two-column signal columns (spread/ratio/cross-above/cross-below), fixed-width `groupByCount`/`groupBySum`/`groupByMin`/`groupByMax`/`groupByMean` plus one-pass single/multi-key `groupByStats`/`groupByStatsOn` and `groupByProfile`/`groupByProfileOn` moment profiles (count/sum/mean/variance/stddev/skewness/kurtosis), fixed-width `innerJoinOn`/`leftJoinOn`/`fullJoinOn`/`semiJoinOn`/`antiJoinOn` for multi-key equi joins plus single-key `innerJoin`/`leftJoin`/`fullJoin`/`semiJoin`/`antiJoin` and ordered `asofJoin`, select/head/tail/take/filter, device transfer, Boltha/Arrow schema/record-batch/table export, Boltha/Parquet byte round-trip, range-pruned reads, lazy `DeviceParquetScan`, and lazy Parquet scalar-filter/projection pushdown, and round-trip conversion for legacy fixed-width dataframe columns.
...
lazy `rollingProfile`, lazy `rollingMomentProfile`, lazy `lagProfile`, lazy `expandingProfile`, lazy `expandingMomentProfile`,
lazy `standardizeProfile`, lazy `robustProfile`, lazy `drawdownProfile`,
lazy `rollingDrawdownProfile`,
lazy `trendProfile`, lazy `extremaProfile`, lazy `rollingRangeProfile`, lazy `crossoverProfile`,
lazy `linearFitProfile`, lazy `rollingLinearFitProfile`, lazy `errorProfile`, lazy `rollingCorrelationProfile`, lazy `validityProfile`,
lazy `classificationProfile`,
lazy `boolTransitionProfile`,
lazy `rollingBoolProfile`,
lazy `signProfile`,
lazy `clipProfile`, lazy `thresholdProfile`,
lazy `leadProfile`,
lazy `rollingNormalizeProfile`,
lazy `rollingQuantileProfile`,
lazy `emaProfile`,
lazy `bucketProfile`,
