...
select/dtype-select/row-index/rename/drop/boolean-column-filter/scalar-filter/filter-mask/sort/slice/take/head/tail operations over either an eager
...
Eager schema/expression helpers such as `selectByDTypes`, `selectNumeric`, `withRowIndex`,
`renameColumn`, `dropColumn`, `compareColumnScalar`, `addColumns`, `filterColumn`, and `filterColumnMask` reuse Vectra `Array` operations, so supported dtypes route
