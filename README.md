...
columns, while `withRowL2Normalize` and `withRowL2Normalized` scale each row's
valid numeric vector to unit L2 norm. `withRowSoftmax` appends one probability
