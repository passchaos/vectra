...
boolean reductions with nullable output at null input positions. Special-float
predicate reductions such as `withRowAnyNaN`/`withRowAllFinite` and their
cumulative/prefix variants extend the same nullable row-wise any/all semantics
to NaN, Inf, signed Inf, finite, normal, subnormal, and non-finite checks.
`withRowCumulativeFirstTrueIndex`/`withRowPrefixFirstTrueIndex`,
