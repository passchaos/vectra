...
payloads nullable, leaving a stable API seam for Axiom hash-join lowering.
`innerJoinOn` and `leftJoinOn` accept multiple fixed-width key columns with
matching dtype pairs, while the other join flavors currently expose the
single-key convenience API.
