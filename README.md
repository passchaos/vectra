...
host mask.
`coalesceColumnsMany`/`coalesceManyColumns`/`coalesceFirstValidColumns` extend
binary `coalesceColumns` to SQL-style first-valid selection across any non-empty
same-dtype column list, with lazy projection pushdown preserving every input
dependency. Sorting follows the cuDF/Polars pattern of producing a stable row order and then
