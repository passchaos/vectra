...
`groupByStats`, and multi-key `groupByStatsOn`), lazy `innerJoinOn`/`leftJoinOn`/`fullJoinOn`/`semiJoinOn`/`antiJoinOn` plus ordered `asofJoin`, lazy `concatRows`/`appendRows`/`vstack`, lazy `distinctRows`/`distinctOn`,
