...
`groupByFirst`/`Last`, `groupByNUnique`, `groupByMode`, `groupByMedian`/`groupByQuantile`, `groupByVariance`/`Stddev`, and their multi-key `*On` variants, `groupByStats`,
...
stable first-tie `groupByMode`/`groupByModeOn`, interpolated `groupByMedian`/`groupByMedianOn` plus configurable `groupByQuantile`/`groupByQuantileOn`, and population `groupByVariance`/`groupByVarianceOn` plus `groupByStddev`/`groupByStddevOn`,
