...
`groupByFirst`/`Last`, `groupByNUnique`, `groupByMode`, `groupByMedian`/`groupByQuantile`, `groupByVariance`/`Stddev`, `groupBySkewness`/`Kurtosis`, bool `groupByAny`/`All` plus `groupByTrueCount`/`FalseCount`/`TrueRatio`/`FalseRatio`, value-validity `groupByValidCount`/`NullCount`/`ValidRatio`/`NullRatio`, and their multi-key `*On` variants, `groupByStats`,
...
stable first-tie `groupByMode`/`groupByModeOn`, interpolated `groupByMedian`/`groupByMedianOn` plus configurable `groupByQuantile`/`groupByQuantileOn`, and population `groupByVariance`/`groupByVarianceOn` plus `groupByStddev`/`groupByStddevOn`, shape `groupBySkewness`/`groupBySkewnessOn` plus `groupByKurtosis`/`groupByKurtosisOn`, bool `groupByAny`/`groupByAnyOn` plus `groupByAll`/`groupByAllOn`, true/false count-ratio `groupByTrueCount`/`groupByTrueCountOn`/`groupByFalseCount`/`groupByFalseCountOn`/`groupByTrueRatio`/`groupByTrueRatioOn`/`groupByFalseRatio`/`groupByFalseRatioOn`, and value-validity `groupByValidCount`/`groupByValidCountOn` plus `groupByNullCount`/`groupByNullCountOn` and `groupByValidRatio`/`groupByValidRatioOn` plus `groupByNullRatio`/`groupByNullRatioOn`,
