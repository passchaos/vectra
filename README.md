...
select/boolean-column-filter/scalar-filter/filter-mask/sort/slice/head/tail operations over either an eager
dataframe source or a `DeviceParquetScan` source, exposes `explain()`, folds
adjacent select/head/tail operations and exposes lazy row slicing, rewrites adjacent sort+head into `topKBy`,
