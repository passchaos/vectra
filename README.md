...
- `ArrayView(T)` can now directly call object-style materializing wrappers for `softmax/logSoftmax`, `norm`, `sort/sortBy/sortDescending`, `argsort/argsortAxis`, `topk`, `matmul/matmulArray`, `dot`, `outer`, `trace/traceOffset`, `diagonal/diag/diagflat`, `triu/tril`, and complex helpers (`real`, `imag`, `conj`, `magnitude`) while preserving non-copying indexing/mutation for view operations.
