...
select/name-pattern-select/name-pattern-drop/dtype-select/dtype-drop/cast/fill-null/drop-null/literal-column/positioned-literal-column/copy-column/row-index/rename/move/drop/boolean-column-filter/scalar-filter/filter-mask/sort/reverse/slice/take/head/tail operations over either an eager
dataframe source or a `DeviceParquetScan` source, exposes `explain()`, folds
adjacent select/head/tail operations and exposes lazy row reversal/slicing/gathering, rewrites adjacent sort+head into `topKBy`,
...
`withRowIndex`, `renameColumn`, `moveColumn`, `moveColumnBefore`, `moveColumnAfter`, `dropColumn`, `compareColumnScalar`, `addColumns`, `filterColumn`, and `filterColumnMask` reuse Vectra `Array` operations, so supported dtypes route
