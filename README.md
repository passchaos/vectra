...
count descending, while `groupByHeadRows`/`groupByHeadRowsOn` and `groupByTailRows`/`groupByTailRowsOn` select stable per-group row slices, and `groupByCount`/`groupByCountOn` plus numeric `groupBySum`/`groupBySumOn`, `groupByProd`/`groupByProdOn`,
