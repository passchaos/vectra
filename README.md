...
- `Series(T)` and heterogeneous CPU `DataFrame` with select/filter/sort/head/tail/describe/group-by-sum, plus a cuDF/Polars-inspired `DeviceDataFrame`/`DeviceColumn` owning fixed-width columns on a shared Vectra device (CPU/CUDA/MPS) with non-owning table/column views, nullable validity metadata, eager same-dtype column arithmetic/comparisons, generated boolean-mask filtering, stable argsort/sort-by-key with null placement, single-key fixed-width `groupByCount`/`groupBySum`, single-key fixed-width `innerJoin`/`leftJoin`, select/head/tail/take/filter, device transfer, Boltha/Arrow schema/record-batch/table export, Boltha/Parquet byte round-trip and range-pruned reads, and round-trip conversion for legacy fixed-width dataframe columns.
...
`innerJoin`/`leftJoin` similarly follow cuDF's hash-join shape: build/probe row
index pairs, gather both tables, and then concatenate payload columns.  The
