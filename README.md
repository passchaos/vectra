...
with `vx.CudaDeviceOperation`, infer partition launch grids, drive family-selected CUDA launches, model loaded CUDA module bundles, carry CUDA device-model contracts, and model CUDA graph capture/replay, and schedule lazy CUDA memory copies without owning
