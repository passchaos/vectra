...
so a Vectra smoke or downstream array backend can build host+kernel launch
metadata with `vx.CudaKernel(Decl).call1D(...)` / `vx.cudaCallWith(...)` without
owning the kernel facade itself.
