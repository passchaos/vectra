...
select/dtype-select/cast/fill-null/drop-null/literal-column/row-index/rename/drop/boolean-column-filter/scalar-filter/filter-mask/sort/slice/take/head/tail operations over either an eager
dataframe source or a `DeviceParquetScan` source, exposes `explain()`, folds
adjacent select/head/tail operations and exposes lazy row slicing/gathering, rewrites adjacent sort+head into `topKBy`,
