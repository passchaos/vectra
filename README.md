...
The current public evidence API is `vx.axiom_backend.lowerMatmulDialect(...)`, which reports registered linalg/memref/gpu dialect counts, operation-store fingerprints, schedule fingerprints, CUDA Tile/NVVM handoff fingerprints for CUDA, and an explicit `planned_mps` status for the MPS backend while Axiom's MPS runtime ABI report remains planned/unavailable. These lowering helpers now validate structural array contracts separately from eager runtime storage contracts, so CPU arrays and CUDA-tagged/device arrays can both describe the same linalg/memref/gpu program while runtime capability reports remain honest. `vx.axiom_backend.lowerElementwiseDialect(...)` reports the same linalg.generic/schedule/vector/gpu route for same-shape add/sub/mul/div; `vx.axiom_backend.lowerReductionDialect(...)` reports linalg.generic/vector/gpu reductions for row/column sum/max/min/prod while `vx.axiom_backend.reductionRuntimeCapability(.cuda)` explicitly reports CUDA reductions as `lowering_only` until Axiom exposes an eager reduction runtime ABI; `vx.axiom_backend.lowerBroadcastAddDialect(...)` reports row/column broadcast-add lowering, `vx.axiom_backend.broadcastAddRuntimeCapability(.cpu)` reports executable CPU row/column bias-add through Axiom CPU→Veyra, and `broadcastAddRuntimeCapability(.cuda)` remains `lowering_only`; `vx.axiom_backend.lowerUnaryDialect(...)` reports linalg.generic copy/square/cube unary routes while `vx.axiom_backend.unaryRuntimeCapability(.cuda, .square)` reports executable CUDA square via device elementwise multiply and other dialect-only unary variants remain capability-gated; `vx.axiom_backend.lowerTransposeDialect(...)` reports 2D transpose lowering while `vx.axiom_backend.transposeRuntimeCapability(.cuda)` reports `lowering_only`; and `vx.setDefaultDialectBackend(.cpu/.cuda/.mps)` switches the default dialect-lowering target dynamically for helpers that use the default route, and CPU eager Array dispatch maps that selector to the corresponding Axiom backend policy with planned MPS falling back to real CPU execution until the MPS ABI becomes available.


## Axiom accelerator backend

Vectra imports the sibling [`../axiom`](../axiom) package by default. Supported
CPU-backed `Array(f32/f64)` same-shape and scalar/broadcast add/sub/mul/div plus square,
Axiom-composed `powScalar(0/1/2/3)`, 2D row/column broadcast-add, 2D axis reductions (`sum/prod/min/max`), 2D transpose, 2D matmul, matrix-vector, vector-matrix, dot/vdot, trace, determinant, inverse,
...
f32 add/sub/mul/div/maximum/minimum/addcmul/addcdiv/lerp/neg/abs/reciprocal/square/sqrt/rsqrt/exp/relu/threshold/leakyRelu/relu6/clip/clipArray/elu/celu/sigmoid/silu/hardsigmoid/hardswish/softsign/softshrink/powScalar(0/1/2/3)/mseLoss(.none)/l1Loss(.none)/smoothL1Loss(.none)/huberLoss(.none), f32 matmul/fused matmul+add, f64 same-shape/scalar elementwise plus maximum/addcmul/addcdiv/lerp/neg/abs/reciprocal/square/sqrt/rsqrt/exp/relu/threshold/leakyRelu/relu6/clip/clipArray/elu/celu/sigmoid/silu/hardsigmoid/hardswish/softsign/softshrink/powScalar(0/1/2/3)/mseLoss(.none)/l1Loss(.none)/smoothL1Loss(.none)/huberLoss(.none), DGEMM matmul/matmulAdd, and f16 and BFloat16 matmul/fused
matmul+add plus widened elementwise/activation/powScalar(0/1/2/3) combinations such as relu/sigmoid/softsign/clip use existing device pointers through Axiom CUDA. Large f32 GEMM and
