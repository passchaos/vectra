...
`groupByFirst`/`Last`, and their multi-key `*On` variants, `groupByStats`,
...
`groupByMean`/`groupByMeanOn`, and order-aware `groupByFirst`/`groupByFirstOn` plus
`groupByLast`/`groupByLastOn`, alongside one-pass single/multi-key `groupByStats`/`groupByStatsOn`
