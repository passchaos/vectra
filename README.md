...
`groupByFirst`/`Last`, `groupByNUnique`, `groupByMode`, `groupByMedian`/`groupByQuantile`, and their multi-key `*On` variants, `groupByStats`,
...
stable first-tie `groupByMode`/`groupByModeOn`, and interpolated `groupByMedian`/`groupByMedianOn` plus configurable `groupByQuantile`/`groupByQuantileOn`,
