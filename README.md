...
`groupByFirst`/`Last`, `groupByNUnique`, `groupByMode`, `groupByMedian`/`groupByQuantile`, `groupByIqr`/`Mad`, `groupByVariance`/`Stddev`/`Sem`/`Cv`/`Fano`, magnitude `groupByMeanAbs`/`MeanSquare`/`Rms`/`L1Norm`/`L2Norm`/`MaxAbs`/`MinAbs`, `groupByGeometricMean`/`HarmonicMean`, stable-log `groupByLogSumExp`/`LogMeanExp`, range `groupByPtp`/`Midrange`/`RangeCoeff`, `groupBySkewness`/`Kurtosis`, bool `groupByAny`/`All` plus `groupByTrueCount`/`FalseCount`/`TrueRatio`/`FalseRatio`, value-validity `groupByValidCount`/`NullCount`/`ValidRatio`/`NullRatio`, `groupByArgMin`/`ArgMax`, and their multi-key `*On` variants, `groupByStats`,
