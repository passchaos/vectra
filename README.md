...
`totalNbytes`, `dataMemoryUsage`, `validityMemoryUsage`, `memoryUsage`, `estimatedSize`, `columnSchema`/`columnSchemaAt`/`columnSchemas`/`schema`/`schemaSummary` with per-column dtype/null-validity/memory/device/schema-compatibility helpers, `equals`/`frameEquals`, `allClose`/`frameAllClose`, `schemaEquals`/`sameSchema`/`schemaCompatible`, `deviceBackendName`, `isCpu`,
