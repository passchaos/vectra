...
with `vx.CudaDeviceOperation`, infer partition launch grids, drive family-selected CUDA launches, model loaded CUDA module bundles, carry CUDA device-model contracts, and model CUDA graph capture/replay/scope nodes, schedule lazy CUDA memory copies, carry host-side CUDA tensor metadata, preserve CUDA device-intrinsic contracts, and carry CUDA execution-context metadata without owning
