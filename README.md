...
and their multi-key `*On` variants, `groupByStats`, multi-key `groupByStatsOn`, and grouped moment profiles through
...
`groupByMin`/`groupByMinOn`, `groupByMax`/`groupByMaxOn`, and
`groupByMean`/`groupByMeanOn` plus one-pass single/multi-key `groupByStats`/`groupByStatsOn`
