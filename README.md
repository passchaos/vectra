...
select/name-pattern-select/name-pattern-drop/dtype-select/dtype-drop/cast/fill-null/drop-null/literal-column/row-index/rename/drop/boolean-column-filter/scalar-filter/filter-mask/sort/reverse/slice/take/head/tail operations over either an eager
...
`fillNullColumn`, `dropNulls`, `withColumnLiteral`, `withRowIndex`, `renameColumn`, `dropColumn`, `compareColumnScalar`, `addColumns`, `filterColumn`, and `filterColumnMask` reuse Vectra `Array` operations, so supported dtypes route
