...
`projectionMetadataNbytes`, `predicateMetadataNbytes`, `pushdownMetadataNbytes`, `validateProjection`/`validatePredicate`/`validatePushdown`/`pushdownValid` (including range-predicate dtype and null-predicate nullability checks),
...
`select`/`appendSelect`/`dropSelected`/`selectAll`/`selectExcept`/`intersectSelect`/`whereRange`/`whereMin`/`whereMax`/`whereBetween`/`whereGe`/`whereLe`/`whereEq`/`whereNull`/`whereIsNull`/`whereIsNotNull`/`whereNotNull`/`collect`/`lazy`/`explain`, and
