...
select/dtype-select/cast/fill-null/literal-column/row-index/rename/drop/boolean-column-filter/scalar-filter/filter-mask/sort/slice/take/head/tail operations over either an eager
...
`fillNullColumn`, `withColumnLiteral`, `withRowIndex`, `renameColumn`, `dropColumn`, `compareColumnScalar`, `addColumns`, `filterColumn`, and `filterColumnMask` reuse Vectra `Array` operations, so supported dtypes route
