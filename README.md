...
`withRowMinMaxScale` and `withRowMinmaxScale` append per-row min-max scaled
...
append signed row-sum shares, and `withRowMaxAbsNormalize`/`withRowMaxabsNormalize`
/`withRowLInfNormalize`/`withRowLinfNormalize` scale by the row maximum absolute value. `withRowSoftmax` appends one probability
