...
`withRowCumulativeNullCount`/`withRowCumNullCount`/`withRowPrefixNullCount`
and `withRowCumulativeValidCount`/`withRowCumValidCount`/
...
boolean reductions with nullable output at null input positions.
`withRowCumulativeFirstTrueIndex`/`withRowPrefixFirstTrueIndex`,
...
positive, and negative row diagnostics. `withRowFirstNaNIndex`/
`withRowFirstNanIndex`, `withRowLastNaNIndex`/`withRowLastNanIndex`,
`withRowFirstInfIndex`/`withRowLastInfIndex`, `withRowFirstFiniteIndex`/
`withRowLastFiniteIndex`, `withRowFirstNonFiniteIndex`/
...
horizontal positions of the first/last NaN, Inf, finite, non-finite, zero,
non-zero, positive, or negative cell in each row.
