...
- `Series(T)` and heterogeneous CPU `DataFrame` with select/filter/sort/head/tail/describe/group-by-sum, plus a cuDF/Polars-inspired `DeviceDataFrame`/`DeviceColumn` owning fixed-width columns on a shared Vectra device (CPU/CUDA/MPS) with non-owning table/column views, nullable validity metadata, eager same-dtype column arithmetic/comparisons, generated boolean-mask filtering, lightweight `DeviceLazyFrame` select/scalar-filter/filter-mask/sort/head/tail plans with explain/adjacent-op/top-k optimization, stable argsort/sort-by-key/top-k with null placement, fixed-width `groupByCount`/`groupBySum`/`groupByMin`/`groupByMax`/`groupByMean` plus one-pass single/multi-key `groupByStats`/`groupByStatsOn`, fixed-width `innerJoinOn`/`leftJoinOn`/`fullJoinOn`/`semiJoinOn`/`antiJoinOn` for multi-key equi joins plus single-key `innerJoin`/`leftJoin`/`fullJoin`/`semiJoin`/`antiJoin` and ordered `asofJoin`, select/head/tail/take/filter, device transfer, Boltha/Arrow schema/record-batch/table export, Boltha/Parquet byte round-trip, range-pruned reads, and lazy `DeviceParquetScan`, and round-trip conversion for legacy fixed-width dataframe columns.
...
`DeviceLazyFrame` stages select/scalar-filter/filter-mask/sort/head/tail operations, exposes
`explain()`, folds adjacent select/head/tail operations, rewrites adjacent
sort+head into `topKBy`, and executes through `collect()` today, preserving a
...
`fromParquetBytesPruned` nexts fixed-width numeric range predicates to
