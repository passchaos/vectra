...
lazy `emaProfile`,
lazy `bucketProfile`,
lazy `innerJoinOn`/`leftJoinOn`/`fullJoinOn`/`semiJoinOn`/`antiJoinOn` plus ordered `asofJoin`, lazy `concatRows`/`appendRows`/`vstack`, lazy `distinctRows`/`distinctOn`/`distinctRowsLast`/`distinctOnLast`/`distinctRowsNone`/`distinctOnNone`, lazy `withRowIsDuplicated`/`withRowIsUnique`,
