...
also expose grouped `vx.ArrowExport.DataFrame.Parquet` helpers plus grouped `vx.ArrowExport.ParquetScan` namespaces (`Lifecycle`, `Device`, `Source`, `File`, `Pushdown`, and `Arrow`) while retaining flat aliases, and `vx.ArrowExport.DataFrame.hasArrowProjection`/`toArrow*`/`fromArrow*`/`fromParquet*`, `Column.toArrow*`,
