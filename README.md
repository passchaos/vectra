...
The CUDA-enabled commands require a CUDA/libnvvm/PTXAS-capable host; `-Daxiom-cuda-dispatch=true` additionally lets ordinary `Array(f32).add/sub/mul/div/addScalar/mulScalar/divScalar/matmul` methods use `vx.axiom_backend` policy to try Axiom CUDA/Axiom CPU before falling back to CPU.  The f32 CUDA matmul path now builds Axiom CUDA Tile IR and hands it to Axiom's Tile-IR-to-CUTILE GEMM runtime bridge.  `Array(f16)` and `Array(BFloat16)` have native Axiom CUDA same-shape elementwise seeds plus Axiom-owned widened GEMM runtime seeds for contiguous 2D matmul; the smoke bridge also reports Axiom typed SIMT GEMM launch-plan readiness metadata for those dtypes.  See [`docs/AXIOM_CUDA_BRIDGE.md`](docs/AXIOM_CUDA_BRIDGE.md) and [`docs/CUDA_DTYPE_SUPPORT.md`](docs/CUDA_DTYPE_SUPPORT.md) for the supported surface, local CUDA dtype matrix, and current limits.
