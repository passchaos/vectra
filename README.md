...
fixed-width tables by rows while preserving nullable validity, and the same op is
available in lazy plans.
...
`groupByStats`, and multi-key `groupByStatsOn`), lazy `innerJoinOn`/`leftJoinOn`/`fullJoinOn`/`semiJoinOn`/`antiJoinOn` plus ordered `asofJoin`, lazy `concatRows`/`appendRows`/`vstack`,
