...
- CUDA `Array(f32).add/sub/mul/div/maximum/minimum/addcmul/addcdiv/lerp/neg/abs/reciprocal/square/sqrt/rsqrt/exp/relu/threshold/leakyRelu/relu6/clip/clipArray/elu/celu/sigmoid/silu/hardsigmoid/hardswish/softsign/softshrink/powScalar(0/1/2/3)/mseLoss(.none)/l1Loss(.none)/smoothL1Loss(.none)/huberLoss(.none)` and `Array(f64).maximum/addcmul/addcdiv/lerp/neg/abs/reciprocal/square/sqrt/rsqrt/exp/relu/threshold/leakyRelu/relu6/clip/clipArray/elu/celu/sigmoid/silu/hardsigmoid/hardswish/softsign/softshrink/powScalar(0/1/2/3)/mseLoss(.none)/l1Loss(.none)/smoothL1Loss(.none)/huberLoss(.none)` use Axiom device unary/elementwise
  elementwise kernels. f16 and BFloat16 widened activation/powScalar combinations such as `relu/sigmoid/softsign/clip/powScalar(0/1/2/3)` are covered by the CUDA device smoke.
...
- General broadcast/reduction/transpose CUDA eager execution, softmax, random CUDA creation, and
  CUDA view storage are not exposed yet. CUDA reduction, broadcast-add, and transpose dialect lowerings are visible through
  `vx.axiom_backend.lowerReductionDialect(...)`, `lowerBroadcastAddDialect(...)`, and `lowerTransposeDialect(...)`, but
