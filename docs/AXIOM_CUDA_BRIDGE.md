...
- General non-f32/f64 reduction/broadcast/transpose CUDA eager execution, softmax, random CUDA creation, and
  CUDA view storage are not exposed yet. CUDA f32/f64 2D `sum/prod/min/max(axis=0/1)` has an
  eager Axiom runtime; non-f32/f64 CUDA reductions, non-f32/f64/general broadcast-add, and non-f32/f64/general transpose
