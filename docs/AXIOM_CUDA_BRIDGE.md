...
contract adapters (`vx.CudaKernelDeviceContract`), CUDA graph adapters (`vx.CudaGraphCapture`, `vx.CudaGraphExecutable`), memory-copy adapters (`vx.CudaMemoryCopyOperation`), tensor/mapped-partition metadata adapters (`vx.CudaTensor`, `vx.CudaMappedPartitionedDeviceSlice`), device-intrinsic contract adapters (`vx.CudaDeviceIntrinsicSet`), and execution-context
