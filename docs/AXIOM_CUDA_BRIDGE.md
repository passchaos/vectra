...
- CUDA `Array(f32/f64/f16/BFloat16).matmul` builds an Axiom memref-backed
  ...
  `Array(f32/f64/f16/BFloat16).bmm` and N-D `matmul` cases whose leading batch
  dimensions are either equal or whole-operand broadcastable now call that Axiom
  batched GEMM runtime, flattening representable leading batch axes at the
  memref boundary rather than falling back to a host loop; fused pack/unpack
  kernels and mixed per-axis batch broadcast legalization remain future Axiom
  lowering work.
