...
- General broadcast/most-reduction/transpose CUDA eager execution, softmax, random CUDA creation, and
  CUDA view storage are not exposed yet. CUDA f32 2D `sum/prod/min/max(axis=0/1)` has an
  eager Axiom runtime; other CUDA reductions, broadcast-add, and transpose
  ...
  `broadcastAddRuntimeCapability(.cuda)` and `transposeRuntimeCapability(.cuda)`
  still report `lowering_only` until Axiom exposes matching eager runtime ABIs.
