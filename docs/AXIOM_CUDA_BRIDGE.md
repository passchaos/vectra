...
contract adapters (`vx.CudaKernelDeviceContract`), CUDA graph adapters (`vx.CudaGraphCapture`, `vx.CudaGraphExecutable`), memory-copy adapters (`vx.CudaMemoryCopyOperation`), and tensor metadata adapters
(`vx.CudaTensor`) only as adapter conveniences; the host+kernel facade remains owned by Axial and
