...
- General non-f32/f64-broadcast/most-reduction/non-f32-transpose CUDA eager execution, softmax, random CUDA creation, and
  CUDA view storage are not exposed yet. CUDA f32 2D `sum/prod/min/max(axis=0/1)` has an
  eager Axiom runtime; other CUDA reductions, non-f32/f64/general broadcast-add, and non-f32/general transpose
