...
- `sameStructure()/dotSameStructure()`：COO 精确结构比较与同结构存储值点积；`coalesced()`：返回按 `(row, col)` 排序且重复坐标已聚合的 COO 副本，便于生成规范化 sparse 结构。
...
- `CsrMatrix.sameStructure()/dotSameStructure()`：CSR 精确压缩结构比较与同结构存储值点积。
...
- `CscMatrix.sameStructure()/dotSameStructure()`：CSC 精确压缩结构比较与同结构存储值点积。
