...
- 后续继续推进 GPU backend 时，优先补 general broadcast、reductions、更多 dtype/device kernel、device cache/pool 策略和 view/device storage 语义；当前 CUDA reductions 只有 Axiom linalg/schedule/vector/gpu lowering 证据，`vx.axiom_backend.reductionRuntimeCapability(.cuda)` 必须报告 `lowering_only`，直到 Axiom 暴露 eager reduction runtime ABI；不能退回“创建在 CPU、计算时来回拷贝”的实现。
