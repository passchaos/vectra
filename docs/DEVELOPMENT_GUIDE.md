...
- 形状：对象/类型方法 `reshape/view`、`reshapeInfer/viewInfer`（单个 `-1` 推断维度）、`flatten/ravel`、`flattenAxes`、`unflatten`、`squeeze/unsqueeze`、`transpose`、`permute`、`swapaxes`、`movedim`、`broadcastTo`、`repeat`、`repeatInterleave/repeatInterleaveScalar`、`tile`、`slice/sliceAxis/slice1d`、`split/chunk`、`hstack/vstack/dstack/columnStack`、`flip/flipAxes`、`roll/rollAxes`、`padConstant/padEdge/padReflect/padWrap/padSymmetric`；非复制视图基础包括 `ArrayView/NDArrayView`、`asView`、安全 `asStrided`、PyTorch-like `unfold`、`sliceAxisView/sliceView`、`selectView/narrowView`、`permuteView/transposeView`、`broadcastView`、共享存储 mutation 与 `toArray/contiguous` materialization。
