...
- 数学/NN：`neg/negative`、`positive`、`abs/absolute/fabs`、`astype`、promoted mixed-dtype variants（`addPromote/subPromote/mulPromote/divPromote/maximumPromote/minimumPromote`）、`square`、`reciprocal`、`sign/signbit`、`nextAfter/nextafter`、`ldexp`、`frexp`、`exp/exp2/expm1`、`log/log1p/log2/log10/lgamma/gammaln`、`sqrt/rsqrt/cbrt`、`floor`、`ceil`、`round`、`trunc`、`deg2rad/radians`、`rad2deg/degrees`、`sinc`、`logit`、`expit`、`sin`、`cos`、`tan`、`asin/arcsin`、`acos/arccos`、`atan/arctan`、`atan2/arctan2`、`hypot`、`copysign`、`heaviside`、`sinh`、`cosh`、`tanh`、`asinh/arcsinh`、`acosh/arccosh`、`atanh/arctanh`、`relu/leakyRelu`、`elu/celu`、`sigmoid`、`silu/SiLU/swish`、`mish`、`hardsigmoid/hardSigmoid`、`hardswish/hardSwish`、`softplus`、`softsign`、`gelu`、`softmax`、`logsumexp`、`logcumsumexp`、`logSoftmax/log_softmax`、`nllLoss/nll_loss`、`crossEntropy/cross_entropy`、`binaryCrossEntropy/binary_cross_entropy`、`binaryCrossEntropyWithLogits/binary_cross_entropy_with_logits`（`LossReduction.none/sum/mean`）、`clip/clamp`、`clipMin/clipMax/clampMin/clampMax`、`lerp`、`addcmul/addcdiv`、`fmax/fmin`、`isNan/isnan`、`isInf/isinf`、`isPosInf/isposinf`、`isNegInf/isneginf`、`isFinite/isfinite`、`isNormal/isnormal`。
...
- CPU-backed `Array(f32/f64)` add/sub/mul/div、scalar/one-element scalar broadcast、2D matmul、matrix-vector、vector-matrix、dot/vdot、trace、det/inverse/solve/cholesky/qr/lu/solveTriangular、matrixNorm(.fro/.one/.inf)、svd 通过 Axiom CPU lowering 到 Veyra。
- 只有 CUDA driver 能 retain 对应 device primary context 时 `Device.cuda(index).isAvailable()` 才为 true；`Array.*On(..., vx.cuda(i))`、确定性 `Context.*With(vx.onDevice(...))` 创建 helper 和 `.cuda(i)` 必须直接分配/持有 device storage，不能只改 device 标签。
- CUDA owning `Array(f32)` 的 same-device `add/sub/mul/div`、contiguous 2D `matmul` 和 `vx.matmulAdd` 必须直接使用已有 device pointer；大 GEMM/GEMM+add 走 Axiom cached cuBLAS SGEMM（`matmulAdd` 使用 `beta=1` 融合 addend），不能把数据拷回 host 再重新上传。`ArrayView.cuda()` 仍未实现，应返回 `InvalidDevice`。
- CPU-backed Axiom bridge 覆盖 `Array(f32/f64)` same-shape add/sub/mul/div、scalar-broadcast、matmul、matrix-vector、vector-matrix、dot/vdot、trace、det/inverse/solve/cholesky/qr/lu/solveTriangular、matrixNorm(.fro/.one/.inf)、svd 和 1D positive-stride view add/sub/mul/div host-slice provenance，并通过 Axiom `TensorDeviceBufferPlan` / `TensorDeviceCopyPlan` 记录 logical elements、required span、byte counts、linear-copy 状态和 fingerprints。
- 验证命令：`zig build test`、`zig build axiom-cpu-dispatch-smoke`、`zig build axiom-backend-policy-smoke`、`zig build axiom-cuda-dispatch-smoke`、`zig build axiom-cuda-device-smoke`、CUDA 主机上的 `zig build -Daxiom-cuda-expect=ran axiom-cuda-smoke`，以及性能对齐检查 `zig build -Doptimize=ReleaseFast example-large-matmul-add -- --execute --backend=cuda --require-cuda`。
- 后续继续推进 GPU backend 时，优先补 general broadcast、reductions、更多 dtype/device kernel、device cache/pool 策略和 view/device storage 语义；不能退回“创建在 CPU、计算时来回拷贝”的实现。
