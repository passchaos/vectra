...
- CUDA/MPS owning `Array` 默认不允许静默 host fallback；未覆盖的 device runtime path 应返回显式错误。只有在诊断时传 `-Ddevice-host-fallback=true` 才允许 CUDA/MPS owning arrays 走 host generic fallback。
