...
- Vectra 当前 `rand` 主路径使用 Philox；CUDA `randWith(f32, dims, vx.seededOn(device, seed))`、MPS `randWith(f32/f16/BFloat16, dims, vx.seededOn(device, seed))`、以及 MPS f32 `normalWith/randnWith(..., vx.seededOn(vx.mps(0), seed))` 在对应目标上必须直接写 device storage，不能先 CPU 生成再上传。CPU `uniform/randint` 也使用 Philox；`` 仍主要接入 Alea seeded scalar streams、Alea 分布对象或 Alea 采样工具；设备侧 Philox 分布扩展继续 capability-gate。
