...
- 只有 CUDA driver 能 retain 对应 device primary context 时 `Device.cuda(index).isAvailable()` 才为 true；`Device.mps(index)` 目前作为 Axiom 方言 lowering 的计划后端暴露，`isAvailable()` 为 false，直到 Axiom 拥有真实 Metal/MPS storage/runtime ABI；`Array.*On(..., vx.cuda(i))`、确定性 `Context.*With(vx.onDevice(...))` 创建 helper 和 `.cuda(i)` 必须直接分配/持有 device storage，不能只改 device 标签。
- CUDA owning `Array(f32)` 的 same-device `add/sub/mul/div/maximum/minimum/addcmul/addcdiv/lerp/neg/abs/reciprocal/square/rsqrt/relu/threshold/leakyRelu/relu6/clip/clipArray/elu/celu/sigmoid/silu/hardsigmoid/hardswish/softsign/softshrink/smoothL1Loss(.none)/huberLoss(.none)`、contiguous 2D `matmul` 和 `vx.matmulAdd`，以及 CUDA owning `Array(f64)` same-shape/scalar elementwise、maximum/addcmul/addcdiv/lerp/neg/abs/reciprocal/square/sqrt/rsqrt/exp/relu/threshold/leakyRelu/relu6/clip/clipArray/elu/celu/sigmoid/silu/hardsigmoid/hardswish/softsign/softshrink/smoothL1Loss(.none)/huberLoss(.none)、contiguous 2D DGEMM `matmul` 和 matmulAdd/fusion，必须直接使用已有 device pointer；大 GEMM/GEMM+add 走 Axiom cached cuBLAS/cuBLASLt 路径，不能把数据拷回 host 再重新上传。`ArrayView.cuda()` 仍未实现，应返回 `InvalidDevice`。f16 和 BFloat16 widened relu/sigmoid/softsign/clip combinations 已由 CUDA device smoke 覆盖。
- CPU-backed Axiom bridge 覆盖 `Array(f32/f64)` same-shape add/sub/mul/div、square、scalar-broadcast、2D axis reductions (`sum/prod/min/max`)、2D transpose、matmul、matrix-vector、vector-matrix、dot/vdot、trace、det/inverse/solve/cholesky/qr/lu/solveTriangular、matrixNorm(.fro/.one/.inf/.two/.nuclear)、svd/singularValues/matrixRank/cond/pinv/lstsq、eigh/eigvalsh 和 1D positive-stride view add/sub/mul/div host-slice provenance，并通过 Axiom `TensorDeviceBufferPlan` / `TensorDeviceCopyPlan` 记录 logical elements、required span、byte counts、linear-copy 状态和 fingerprints。
...
- 后续继续推进 GPU backend 时，优先补 general broadcast、reductions、更多 dtype/device kernel、device cache/pool 策略和 view/device storage 语义；当前 CUDA reductions、broadcast-add、transpose 只有 Axiom linalg/schedule/vector/gpu lowering 证据，`vx.axiom_backend.reductionRuntimeCapability(.cuda)`、`broadcastAddRuntimeCapability(.cuda)`、`transposeRuntimeCapability(.cuda)` 必须报告 `lowering_only`，直到 Axiom 暴露对应 eager runtime ABI；不能退回“创建在 CPU、计算时来回拷贝”的实现。
