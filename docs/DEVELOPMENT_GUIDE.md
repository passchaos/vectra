...
- 设备抽象：先保持 CPU 正确；CUDA/GPU API 形态参考 CuPy/PyTorch；当前允许通过 `-Daxiom-cuda=true` 使用 `vx.axiom_cuda` 做 contiguous same-shape `Array(f32)` add/mul/SAXPY/matmul 的 opt-in Axiom CUDA smoke bridge，但 `.cuda()` 持久 device storage 仍不得假装已完成。
...
- 当前覆盖 contiguous same-shape `Array(f32)` add/mul/SAXPY/matmul host-slice bridge，并通过 Axiom `TensorDeviceBufferPlan` / `TensorDeviceCopyPlan` 记录 logical elements、required span、byte counts、linear-copy 状态和 fingerprints。
