...
- `sameStructure()/dotSameStructure()/maxAbsDiffSameStructure()/maxAbsDiffSameStructureMeetsBound()`：COO 精确结构比较、同结构存储值点积与最大绝对差；`coalesced()`：返回按 `(row, col)` 排序且重复坐标已聚合的 COO 副本，便于生成规范化 sparse 结构。
...
- `CsrMatrix.sameStructure()/dotSameStructure()/maxAbsDiffSameStructure()/maxAbsDiffSameStructureMeetsBound()`：CSR 精确压缩结构比较、同结构存储值点积与最大绝对差。
...
- `CscMatrix.sameStructure()/dotSameStructure()/maxAbsDiffSameStructure()/maxAbsDiffSameStructureMeetsBound()`：CSC 精确压缩结构比较、同结构存储值点积与最大绝对差。
