...
- 只有 CUDA driver 能 retain 对应 device primary context 时 `Device.cuda(index).isAvailable()` 才为 true；`Device.mps(index)` 在 macOS 上由 Axiom 的 Metal/MPS runtime ABI 探测真实 Metal device/command queue，并通过 shared `MTLBuffer` 支持 owning `Array` storage、CPU↔MPS copy、same-device copy 和 fill。MPS Array 算子 kernel runtime 仍保持 capability-gated planned 状态，直到 Axiom 补齐 MPSGraph/Metal kernels；`Array.*On(..., vx.cuda(i)/vx.mps(i))`、确定性 `Context.*With(vx.onDevice(...))` 创建 helper 和 `.cuda(i)` / `.mps(i)` 必须直接分配/持有 device storage，不能只改 device 标签。
