...
- 数组数据结构：shape、stride、dtype、device、内存布局、metadata helpers（`dim/rank/numDims`、`nelement`、`shapeAt/strideAt`、`nbytes/elementSize`、`storageOffset/storage_offset`、`dataPtr/data_ptr`、`isEmpty/is_contiguous`、`isMatrix/isSquare/isBatchedMatrix`、`broadcastShape/broadcastShapes/broadcastWith`）和深拷贝 helpers（`clone/copy/detach`）、contiguous/non-contiguous view；当前已有 `ArrayView` / `NDArrayView` 非 owning 视图基础，后续继续把更多 kernel 做成 view-aware。
...
- 形状：对象/类型方法 `reshape/view`、`reshapeInfer/viewInfer`（单个 `-1` 推断维度）、`flatten/ravel`、`flattenAxes`、`unflatten`、`atLeast1d/atLeast2d/atLeast3d`、`squeeze/unsqueeze`、`transpose`、`matrixTranspose/matrix_transpose/mT`、`adjoint/mH/H_`、`matrixPower/matrix_power`、`permute`、`swapaxes`、`movedim`、`broadcastTo/expand/expandAs`、`repeat`、`repeatInterleave/repeatInterleaveScalar`、`tile`、`slice/sliceAxis/slice1d`、`split/chunk`、`hstack/vstack/dstack/columnStack`、`flip/flipAxes/fliplr/flipud`、`roll/rollFlat/rollAxes`、`rot90`、`padConstant/padEdge/padReflect/padWrap/padSymmetric`；非复制视图基础包括 `ArrayView/NDArrayView`、`asView`、安全 `asStrided`、PyTorch-like `unfold`、`sliceAxisView/sliceView`、`selectView/narrowView`、`narrowSigned`、`permuteView/transposeView/T_/matrixTranspose/mT/adjoint/matrixPower/diagonalView/diagonalAxesView/swapaxesView/movedimView`、`view/viewInfer`、`broadcastView/expandView`、`atLeast1d/atLeast2d/atLeast3d`、共享存储 mutation 与 `toArray/contiguous` materialization。
