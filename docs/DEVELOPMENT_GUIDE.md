...
- 形状：对象/类型方法 `reshape/view`、`reshapeInfer/viewInfer`（单个 `-1` 推断维度）、`flatten/ravel`、`flattenAxes`、`unflatten`、`squeeze/unsqueeze`、`transpose`、`permute`、`swapaxes`、`movedim`、`broadcastTo`、`repeat`、`repeatInterleave/repeatInterleaveScalar`、`tile`、`slice/sliceAxis/slice1d`、`split/chunk`、`hstack/vstack/dstack/columnStack`、`flip`、`roll`、`padConstant/padEdge/padReflect/padWrap/padSymmetric`；非复制视图基础包括 `ArrayView/NDArrayView`、`asView`、安全 `asStrided`、PyTorch-like `unfold`、`sliceAxisView/sliceView`、`selectView/narrowView`、`permuteView/transposeView`、`broadcastView`、共享存储 mutation 与 `toArray/contiguous` materialization。
...
- ArrayView 也支持对象式 materializing 索引/搜索/形状包装：`repeatInterleave/repeatInterleaveScalar`、`take/takeSigned`、`indexSelect/indexSelectSigned`、`gather/gatherSigned`、`maskedSelect`、`compress`、`nonzero/argwhere/whereIndices`、`unique`、`searchsorted`、`bucketize`、`digitize`、`isin`。
