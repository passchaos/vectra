...
- CPU-backed `Array(f32/f64)` add/sub/mul/div、scalar/one-element scalar broadcast、2D axis reductions (`sum/prod/min/max`)、2D transpose、2D matmul、matrix-vector、vector-matrix、dot/vdot、trace、det/inverse/solve/cholesky/qr/lu/solveTriangular、matrixNorm(.fro/.one/.inf)、svd 通过 Axiom CPU lowering 到 Veyra。
