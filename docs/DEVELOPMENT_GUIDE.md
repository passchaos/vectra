...
- 组合/矩阵辅助：`cat/concatenate`、`stack`、`outer`、`diagonal`、`trace`、`triu/tril`；`linalg.matmul/matvec/inverse/cholesky/qr/svd/lstsq`。
...
- 底层 f64 dense linalg 优先委托 Veyra；当前 `matmul`、`matvec`、`trace`、`inverse`、`cholesky`、`qr`、`svd`、`lstsq` 已接入 Veyra-compatible 路径；非 f64 或 Veyra 暂无覆盖时保留 Vectra 泛型回退。
