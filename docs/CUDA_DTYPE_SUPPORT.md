...
| --- | --- | --- | --- |
| `CUDA_R_16F` | real half | `f16` | Same-shape add/sub/mul/div, contiguous 2D sum/prod/min/max(axis=0/1), row/column broadcast-add, 2D transpose, and softmax(axis=0/1), and logSoftmax(axis=0/1) now try Axiom's native f16 CUDA runtime seeds first, with widened-to-f32 fallback for same-shape elementwise when needed. Contiguous 2D matmul calls Axiom's typed SIMT GEMM runtime seed, which reports typed launch-plan readiness and the explicit `widened_f32_cuda_compute` route while using widened f32 compute underneath today. |
| `CUDA_C_16F` | complex half pair | Not exposed | Planned. |
| `CUDA_R_16BF` | real bfloat16 | `BFloat16` | Same-shape add/sub/mul/div, contiguous 2D sum/prod/min/max(axis=0/1), row/column broadcast-add, 2D transpose, and softmax(axis=0/1), and logSoftmax(axis=0/1) now try Axiom's native BF16 CUDA runtime seeds first, with widened-to-f32 fallback for same-shape elementwise when needed. Contiguous 2D matmul calls Axiom's typed SIMT GEMM runtime seed, which reports typed launch-plan readiness and the explicit `widened_f32_cuda_compute` route while using widened f32 compute underneath today; CUTILE tensor-core lowering remains future work. |
| `CUDA_C_16BF` | complex bfloat16 pair | Not exposed | Planned. |
| `CUDA_R_32F` | real float | `f32` | Native Axiom CUDA seed for add/sub/mul/div, SAXPY, 2D softmax(axis=0/1), and 2D logSoftmax(axis=0/1); owning CUDA Array matmul/matmulAdd use Axiom cached cuBLAS SGEMM for production throughput with the CUDA Tile IR seed retained as fallback/provenance. |
...
- `Array(f16)` and `Array(BFloat16)` now try Axiom's native typed CUDA
  ...
  reduction, broadcast-add, transpose, and softmax kernels for contiguous 2D sum/prod/min/max(axis=0/1), row/column bias-add, 2D transpose, and softmax(axis=0/1), and logSoftmax(axis=0/1) before falling back to
...
The CUDA smoke JSON includes `strided_add_ok`, `strided_sub_ok`, `strided_mul_ok`, `strided_div_ok`, `strided_abs_ok`, `strided_sqrt_ok`, `strided_exp_ok`, `strided_memref_legality_fingerprint`, `strided_unary_memref_legality_fingerprint`, `strided_scalar_memref_legality_fingerprint`, `strided_scalar_add_ok`, `strided_scalar_sub_ok`, `strided_scalar_mul_ok`, `strided_scalar_div_ok`, `f64_strided_add_ok`, `f64_strided_sub_ok`, `f64_strided_mul_ok`, `f64_strided_div_ok`, `f64_strided_abs_ok`, `f64_strided_sqrt_ok`, `f64_strided_exp_ok`, `f64_strided_memref_legality_fingerprint`, `f64_strided_unary_memref_legality_fingerprint`, `f64_strided_scalar_memref_legality_fingerprint`, `f64_strided_scalar_add_ok`, `f64_strided_scalar_sub_ok`, `f64_strided_scalar_mul_ok`, `f64_strided_scalar_div_ok`, `f16_strided_add_ok`, `f16_strided_sub_ok`, `f16_strided_mul_ok`, `f16_strided_div_ok`, `f16_strided_abs_ok`, `f16_strided_sqrt_ok`, `f16_strided_exp_ok`, `f16_strided_memref_legality_fingerprint`, `f16_strided_unary_memref_legality_fingerprint`, `f16_strided_scalar_memref_legality_fingerprint`, `f16_strided_scalar_add_ok`, `f16_strided_scalar_sub_ok`, `f16_strided_scalar_mul_ok`, `f16_strided_scalar_div_ok`, `bf16_strided_add_ok`, `bf16_strided_sub_ok`, `bf16_strided_mul_ok`, `bf16_strided_div_ok`, `bf16_strided_abs_ok`, `bf16_strided_sqrt_ok`, `bf16_strided_exp_ok`, `bf16_strided_memref_legality_fingerprint`, `bf16_strided_unary_memref_legality_fingerprint`, `bf16_strided_scalar_memref_legality_fingerprint`, `bf16_strided_scalar_add_ok`, `bf16_strided_scalar_sub_ok`, `bf16_strided_scalar_mul_ok`, `bf16_strided_scalar_div_ok`, `direct_softmax_ok`, `direct_log_softmax_ok`, `f16_add_ok`, `f16_matmul_ok`, `bf16_add_ok`,
`bf16_matmul_ok`, `bf16_broadcast_ok`, `bf16_reduction_ok`, `bf16_transpose_ok`, `bf16_softmax_ok`, `bf16_log_softmax_ok`, `f16_broadcast_ok`, `f16_reduction_ok`, `f16_transpose_ok`, `f16_softmax_ok`, `f16_log_softmax_ok`, `f64_matmul_ok`, `f64_elementwise_ok`, `f64_softmax_ok`, `f64_log_softmax_ok`, and `f64_matmul_add_ok` fields when the CUDA smokes run. The device smoke also reports `elementwise_binary_memref_fingerprint`, `elementwise_unary_memref_fingerprint`, `gemm_memref_fingerprint`, `f64_gemm_memref_fingerprint`, `reduction_memref_fingerprint`, `broadcast_memref_fingerprint`, `transpose_memref_fingerprint`, `softmax_memref_fingerprint`, `log_softmax_memref_fingerprint`, and `memref_fingerprints_ok` so CI can prove the eager device paths crossed the Axiom memref-spec boundary instead of calling raw pointer/shape ABIs directly. It also includes
