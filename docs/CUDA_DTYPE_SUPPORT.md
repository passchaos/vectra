...
| --- | --- | --- | --- |
| `CUDA_R_16F` | real half | `f16` | Same-shape add/sub/mul/div and contiguous 2D sum/prod/min/max(axis=0/1) now try Axiom's native f16 CUDA runtime seeds first, with widened-to-f32 fallback for same-shape elementwise when needed. Contiguous 2D matmul calls Axiom's typed SIMT GEMM runtime seed, which reports typed launch-plan readiness and the explicit `widened_f32_cuda_compute` route while using widened f32 compute underneath today. |
| `CUDA_C_16F` | complex half pair | Not exposed | Planned. |
| `CUDA_R_16BF` | real bfloat16 | `BFloat16` | Same-shape add/sub/mul/div and contiguous 2D sum/prod/min/max(axis=0/1) now try Axiom's native BF16 CUDA runtime seeds first, with widened-to-f32 fallback for same-shape elementwise when needed. Contiguous 2D matmul calls Axiom's typed SIMT GEMM runtime seed, which reports typed launch-plan readiness and the explicit `widened_f32_cuda_compute` route while using widened f32 compute underneath today; CUTILE tensor-core lowering remains future work. |
...
- `Array(f16)` and `Array(BFloat16)` now try Axiom's native typed CUDA
  ...
  reduction kernels for contiguous 2D sum/prod/min/max(axis=0/1) before falling back to
...
`bf16_matmul_ok`, `bf16_reduction_ok`, `f16_reduction_ok`, `f64_matmul_ok`, `f64_elementwise_ok`, and `f64_matmul_add_ok` fields when the CUDA smokes run. It also includes
