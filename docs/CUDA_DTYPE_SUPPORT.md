...
- `Array(f16)` and `Array(BFloat16)` now try Axiom's native typed CUDA
  ...
  reduction, broadcast binary, transpose, and softmax kernels for contiguous 2D sum/prod/min/max(axis=0/1), row/column bias binary, 2D transpose, and softmax(axis=0/1), and logSoftmax(axis=0/1) before falling back to
