...
- `einsum`/general contraction syntax: Vectra now has a bounded binary
  ...
  inference, and output reordering, including single-operand transpose/reduction/diagonal/trace forms with batched diagonal/trace, same-label binary elementwise/reduction forms, a three-operand matrix-chain form, and the common batched matmul forms
