...
- Strided/transposed device GEMM lowering: Axiom now supports contiguous and
  ...
  cuBLAS/cuBLASLt leading dimensions.  It still needs bufferization/layout-
  transform lowering for transposed, negative-stride, and batched device GEMM
  layouts.
...
- `einsum`/general contraction syntax: Vectra now has a bounded binary
  ...
  `bij,bjk->bik`, `...ij,...jk->...ik`, and `...i,...i->...`, but full
  NumPy-compatible syntax (general ellipsis, repeated labels, more than two
  operands, and optimized path planning) is not covered yet.
