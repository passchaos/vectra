...
- MPS kernel runtime parity: Axiom now owns a real Metal/MPS device,
  command-queue, shared-buffer storage ABI, executable f32 2D Metal kernels
  ...
  broadcast-add, and sum/prod/min/max reductions, covered by
...
- Strided/transposed device GEMM lowering: Axiom now supports contiguous and
  ...
  copy-pack/GEMM/copy-unpack runtime route, and Vectra CUDA `dot/vdot`, common `inner/outer`, `norm(p=2)`, `bmm`,
...
- `einsum`/general contraction syntax: Vectra now has a bounded binary
  ...
  inference, and output reordering, including single-operand transpose/reduction/diagonal/trace forms with batched diagonal/trace, same-label binary elementwise/reduction forms, 2D and shared-prefix batched three-operand matrix-chain forms, and the common batched matmul forms
