...
pub fn build(b: *std.Build) void {
    ...
    const is_macos_target = target.result.os.tag == .macos;
    ...
    const matmul_materialize_bench_exe = b.addExecutable(.{
        .name = "vectra-matmul-materialize-bench",
        .root_module = b.createModule(.{
            .root_source_file = b.path("tools/bench_matmul_materialize.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{
                .{ .name = "vectra", .module = mod },
    ...
    test_step.dependOn(&axiom_backend_policy_smoke_cmd.step);
    test_step.dependOn(&axiom_device_fallback_policy_smoke_cmd.step);
